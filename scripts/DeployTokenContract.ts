...
async function main() {
  ...
  await gtetTokenContract.mint(voter.address, MINT_VALUE);
  await gtetTokenContract.mint(other.address, MINT_VALUE);

  const deployerDelegateTx = await gtetTokenContract.delegate(deployer.address);
